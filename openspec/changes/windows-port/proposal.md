...
- Make the **Rust core cross-platform**: build the sidecar as `bureaucratpdf-core.exe` for `x86_64-pc-windows-msvc` (cross-compiled from macOS via cargo-xwin and built natively in CI), with the JSON stdin/stdout protocol unchanged. The Windows shell spawns it via `System.Diagnostics.Process`, exactly as the Swift shell spawns it via `Process()`.
