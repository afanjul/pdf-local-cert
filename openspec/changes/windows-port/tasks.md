...
- [x] 1.4 Update the dev-fallback core path in `CoreClient.swift` (`core/target/release/bureaucratpdf-core`) to the new relative location
...
- [x] 2.2 Cross-compile the core from macOS: `cargo xwin build --release --target x86_64-pc-windows-msvc`; confirm `bureaucratpdf-core.exe` is produced
...
- [x] 3.5 End-to-end spike: drive the cross-compiled `bureaucratpdf-core.exe` from the console app — real `prepare` → CNG sign → `finalize` on a test PDF; open the output in Adobe to confirm a valid signature + full chain
...
- [x] 4.2 Port `CoreClient` to C#: spawn `bureaucratpdf-core.exe` via `System.Diagnostics.Process` with redirected stdin/stdout, one request line / one response line, with package + dev-fallback path resolution
...
- [x] 5.1 Add an MSIX packaging project/manifest; bundle `bureaucratpdf-core.exe` and the Windows App SDK runtime
...
- [x] 7.0.1 Add `CommunityToolkit.Mvvm` to `BureaucratPdf.App` (nav uses the built-in `SelectorBar`, so `Segmented` is not needed; `SettingsControls` is deferred to its phase 7.5)
