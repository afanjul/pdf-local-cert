...
final class AppModel {
    ...
    func previewData() -> AppearanceData {
        let qr = appearance.showQR
            ? "https://verify.bureaucratpdf.app/v/\(verifyToken ?? "preview")"
            : nil
    ...
    ///
    /// `pointSize` is the box size in PDF points (the same units the signed
    /// output uses) and is what the font size is relative to — pass it so the
    /// preview's text/box ratio matches the output exactly. When nil, the pixel
    /// size is used as the point size (abstract sidebar thumbnail). `pixelSize`
    /// only drives the bitmap resolution.
    func appearancePreview(pointSize: CGSize? = nil, pixelSize: CGSize) -> NSImage? {
        guard pixelSize.width >= 1, pixelSize.height >= 1 else { return nil }
        let pw = Int(pixelSize.width.rounded()), ph = Int(pixelSize.height.rounded())
        let pts = pointSize ?? pixelSize
        guard pts.width >= 1, pts.height >= 1 else { return nil }
        let data = previewData()
        // Render scale maps PDF points → the requested pixel size (crispness).
        let renderScale = max(1, pixelSize.width / pts.width)
        ...
        let key = "\(pw)x\(ph)|\(Int(pts.width))x\(Int(pts.height))|\(appearance)|\(data.name)|\(data.reason ?? "")|\(data.location ?? "")|\(data.dateString)|\(data.qrPayload ?? "")"
        if let cached = previewCache[key] { return cached }
        let img = AppearanceRenderer.render(
            appearance, data: data,
            pointSize: pts, scale: renderScale)?.nsImage
        if let img {
