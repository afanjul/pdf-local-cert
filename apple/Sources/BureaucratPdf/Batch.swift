...
extension AppModel {
    ...
    var batchSummary: String {
        ...
        return String(format: NSLocalizedString("batch_summary", comment: ""), done, failed, batchItems.count)
    }
...
struct BatchView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            HStack {
                Text(NSLocalizedString("batch_signing", comment: "")).font(.headline)
                Spacer()
                Button(NSLocalizedString("add", comment: "")) { pick() }
                Button(NSLocalizedString("clear", comment: "")) { model.clearBatch() }
            ...
            if model.batchItems.isEmpty {
                DropZone(prompt: NSLocalizedString("drag_multiple_pdfs", comment: ""), isActive: isDropTargeted) { _ in }
            } else {
            ...
            HStack {
                if !model.license.isPro {
                    Label(NSLocalizedString("pro", comment: ""), systemImage: "lock.fill").font(.caption).foregroundStyle(.orange)
                }
                ...
                Button {
                    ...
                    HStack {
                        ...
                        Text(NSLocalizedString("sign_all", comment: ""))
                    }
                }
